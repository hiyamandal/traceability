...
fig.update_layout(height = 250 ,title="Prozessdaten an Station Spanen",  margin = {'t':0, 'b':0, 'l':0}, ) # margin = {'t':0, 'b':0, 'l':0},
...
# plotly_fig_train = app.get_asset_url('train_spanen.png')
# plotly_fig_test = app.get_asset_url('test_spanen.png')

import base64
plotly_fig_train_source = 'train_spanen.png' # replace with your own image
plotly_fig_train = base64.b64encode(open(plotly_fig_train_source , 'rb').read())

app.layout = html.Div(children=[
    html.H1(children='Datenanalyse Station Spanen', style={
        'textAlign': 'center'
    }),

    html.Div(children='Datenanalyse für die Station Spanen.', style={
        'textAlign': 'center',
    }),
    ...
    html.Div([
        html.Div([
            ...
            html.Img(src=app.get_asset_url('train_spanen.png'))
    ...
    html.Div([
        # html.H4('Wirtschaftliche Bewertung'),
        dcc.Graph(
            id='example-graph',
            figure={
                'data': [
                    {'x': [1, 2, 3], 'y': [4, 1, 2], 'type': 'bar', 'name': 'Label1'},
                    {'x': [1, 2, 3], 'y': [2, 4, 5], 'type': 'bar', 'name': u'Label2'},
                ],
                'layout': {
                    'title': '<b> Wirtschaftliche Bewertung <b>'
                }
            }
        ),
    ]),
    html.Div([
        html.H4('Handlungsempfehlung: Weiter zu Station Spanen'),
    ])
